<template lang="pug">
.curso-main-container.pb-3
  BannerInterno(:subTitulo="'4. Diseño y análisis de escenarios futuros'")
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5.bg-tema-4
    .bg-color-4.mb-5(data-aos="fade-up" style="background-color:#F0F0F0 ")
      .row.justify-content-center.align-items-center
        .col-lg.order-2
          .p-4
            p.mb-0(data-aos="fade-up") La construcción de escenarios es una de las herramientas más utilizadas en el análisis prospectivo. Este enfoque fue popularizado por Herman Kahn, pionero de los estudios de prospectiva en la RAND Corporation, durante la década de 1950, y más tarde desarrollado en instituciones como el Instituto de Prospectiva de París. Los escenarios se emplean para explorar futuros alternativos que permitan anticipar riesgos, identificar oportunidades y definir estrategias adaptativas (Kahn & Wiener, 1967).
        .col-lg-auto
          figure
            img(src='@/assets/curso/temas/47.png', alt='')
    p.mb-4 El análisis de escenarios es esencial en sectores que operan en entornos volátiles, puesto que proporciona una visión integral de las dinámicas complejas, permitiendo a los líderes anticipar riesgos, mejorar la resiliencia y desarrollar estrategias innovadoras.
      br
      br
      |Un esquema recomendado para el análisis de escenarios, se plantea en la siguiente tabla:
    .row.justify-content-center.mb-5
      .col-lg-8
        .bg-color-white.shadow-soft.py-4.px-lg-5
          .row.justify-content-around.align-items-center
            .col-md-auto
              img(src="@/assets/template/bg-icono-pdf.svg", style="width: 90px;")
            .col
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  h3.mb-1 Tabla 1
                  p Para comenzar con el estudio de esta temática, lo invitamos a ver el siguiente pdf:
                .col-sm-auto
                  a.boton.color-acento-botones.texto-blanco(:href="obtenerLink('/downloads/Anexos_tabla1.pdf')" target="_blank")
                    span Descargar
                    i.fas.fa-arrow-right
    h3 Aplicaciones prácticas del diseño de escenarios futuros
    .row.justify-content-center.mb-5
      .col-lg-8.mb-lg-0.mb-3
        .p-4(style="background-color: #D9EFF8")
          p.mb-0 El diseño y análisis de escenarios futuros, es una herramienta poderosa para anticipar riesgos y oportunidades en entornos complejos. En el contexto empresarial y organizacional, este enfoque permite visualizar diversas trayectorias posibles y adaptar estrategias que fortalezcan la resiliencia y la innovación. Aplicando este método, las organizaciones no solo se preparan para el cambio, sino que logran influir en su entorno y liderar transformaciones claves. 
      .col-lg-4.col-7: img(src='@/assets/curso/temas/48.png', alt='')
    .row.justify-content-center.mb-3
      .col-lg-6.mb-lg-0.mb-3
        .bg-color-1.p-4.mb-5.h-100.d-flex.align-items-center(data-aos="fade-left" style="background-color: #F8DCF6 ")
          .row.align-items-center
            .col-lg-auto
              img(src="@/assets/curso/temas/35.svg", style="max-width: 90px").mx-auto
            .col-lg
              p.mb-0 El diseño y análisis de escenarios futuros, ofrece múltiples beneficios, permitiendo a las organizaciones prepararse estratégicamente para enfrentar entornos inciertos. 
      .col-lg-6
        p.mb-4 A continuación, se presentan las principales ventajas:
        .bg-color-white.shadow-soft.py-4.px-lg-5
          .row.justify-content-around.align-items-center
            .col-md-auto
              img(src="@/assets/template/bg-icono-pdf.svg", style="width: 90px;")
            .col
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  h3.mb-1 Tabla 2
                  p Para comenzar con el estudio de esta temática, lo invitamos a ver el siguiente pdf:
                .col-sm-auto
                  a.boton.color-acento-botones.texto-blanco(:href="obtenerLink('/downloads/Anexos_tabla2.pdf')" target="_blank")
                    span Descargar
                    i.fas.fa-arrow-right
    .row.justify-content-center.mb-5
      .col-lg-6.mb-lg-0.mb-3
        .bg-color-1.p-4.mb-5.h-100.d-flex.align-items-center(data-aos="fade-left" style="background-color: #FFF8D5 ")
          .row.align-items-center
            .col-lg-auto
              img(src="@/assets/curso/temas/35.svg", style="max-width: 90px").mx-auto
            .col-lg
              p.mb-0 El diseño y análisis de escenarios futuros, ofrece múltiples beneficios, permitiendo a las organizaciones prepararse estratégicamente para enfrentar entornos inciertos. 
      .col-lg-6
        p.mb-4 Y se destacan las limitaciones más relevantes:
        .bg-color-white.shadow-soft.py-4.px-lg-5
          .row.justify-content-around.align-items-center
            .col-md-auto
              img(src="@/assets/template/bg-icono-pdf.svg", style="width: 90px;")
            .col
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  h3.mb-1 Tabla 3
                  p Para comenzar con el estudio de esta temática, lo invitamos a ver el siguiente pdf:
                .col-sm-auto
                  a.boton.color-acento-botones.texto-blanco(:href="obtenerLink('/downloads/Anexos_tabla3.pdf')" target="_blank")
                    span Descargar
                    i.fas.fa-arrow-right
    .p-4(style="background-color: #D9EFF8 ")
      p.mb-0 El diseño y análisis de escenarios futuros, es una metodología fundamental para anticiparse a los desafíos en entornos cambiantes. Las ventajas superan las limitaciones, especialmente cuando se gestiona de forma estratégica y con monitoreo constante. Las organizaciones que adoptan este enfoque desarrollan una mayor capacidad de adaptación, innovación y resiliencia, lo que les permite no solo enfrentar cambios, sino también liderar en su sector. 




</template>

<script>
import BannerInterno from '../../components/plantilla/BannerInterno'
export default {
  name: 'Tema4',
  components: {
    BannerInterno,
  },
  mounted() {
    this.$nextTick(() => {
      this.$aosRefresh()
    })
  },
}
</script>

<style lang="sass">
.material-complementario
  padding-top: 3rem
  border-top: 9px solid $color-sistema-e
</style>
